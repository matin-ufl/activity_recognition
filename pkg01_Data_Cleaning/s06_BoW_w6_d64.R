...
# Selecting V1 to V60 (acceleration data) for clustering
...
save(codebook, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/BoW_W6_D64/codebook_W6_D64.Rdata")

# Labeling the data chunks
rawBoW.df$word.idx <- sapply(as.numeric(cluster.outcome$clustering), FUN = function(x) {as.character(paste("D", x, sep = ""))})
save(rawBoW.df, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/BoW_W6_D64/merged_rawBoW_labeled_W6_D64.Rdata")
...
bow_w6_d64.df <- data.frame(matrix(nrow = 0, ncol = 3 + d))
for(ppt in levels(as.factor(TF.df$PID))) {
     ...
     for(task in levels(as.factor(ppt.df$Task))) {
          ...
          bow_w6_d64.df <- rbind(bow_w6_d64.df, temp.df)
     }
}
rm(ppt, ppt.df, task, task.df, temp.df, met.df, pptTF.df, taskTF.df, i)
remove.idx <- which(is.na(bow_w6_d64.df$D1))
bow_w6_d64.df <- bow_w6_d64.df[-remove.idx, ]

# Dataset is ready! Save it to a file!
save(bow_w6_d64.df, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/BoW_W6_D64/BoW_W6_D64.Rdata")
...
plot.codebook.W6D64(codebook)


# Plotting a sample conversion ------------------------------
task.df <- rawBoW.df[rawBoW.df$PID == "BAVA070", ]
task.df <- task.df[task.df$Task == "WALKING AT RPE 5", ]
