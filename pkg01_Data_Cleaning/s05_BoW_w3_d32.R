...
save(rawBoW.df, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/BoW_W3_D32/merged_rawBoW_labeled_W3_D32.Rdata")
...
bow_w3_d32.df <- data.frame(matrix(nrow = 0, ncol = 3 + d))
for(ppt in levels(as.factor(TF.df$PID))) {
     ...
     for(task in levels(as.factor(ppt.df$Task))) {
          ...
          bow_w3_d32.df <- rbind(bow_w3_d32.df, temp.df)
     }
}
rm(ppt, ppt.df, task, task.df, temp.df, met.df, pptTF.df, taskTF.df, i)
remove.idx <- which(is.na(bow_w3_d32.df$D1))
bow_w3_d32.df <- bow_w3_d32.df[-remove.idx, ]

# Dataset is ready! Save it to a file!
save(bow_w3_d32.df, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/BoW_W3_D32/BoW_W3_D32.Rdata")
...
task.df <- task.df[task.df$Task == "LIGHT GARDENING", ]
