...
numberOfWordsInMinute <- 58 # 58 is for 3-second words
...
save(head_tail_codebook, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/tBoW_W3_D32/head_tail_codebook_W3_D32.Rdata")
...
save(body_codebook, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/tBoW_W3_D32/body_codebook_W3_D32.Rdata")
...
save(rawBoW.df, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/tBoW_W3_D32/merged_raw_tBoW_labeled_W3_D32.Rdata")
rm(new.df)
...
tbow_w3_d32.df <- data.frame(matrix(nrow = 0, ncol = 3 + (d + d_t)))
for(ppt in levels(as.factor(TF.df$PID))) {
     ...
     for(task in levels(as.factor(ppt.df$Task))) {
          ...
          tbow_w3_d32.df <- rbind(tbow_w3_d32.df, temp.df)
     }
}
rm(ppt, ppt.df, task, task.df, temp.df, met.df, pptTF.df, taskTF.df, i)
remove.idx <- which(is.na(tbow_w3_d32.df$D1))
tbow_w3_d32.df <- tbow_w3_d32.df[-remove.idx, ]

# Dataset is ready! Save it to a file!
save(tbow_w3_d32.df, file = "~/Dropbox/Work-Research/Current Directory/Activity Recognition/Datasets/Cleaned Data/tBoW_W3_D32/tBoW_W3_D32.Rdata")
...
plot.codebook.W3D32(head_tail_codebook)
plot.codebook.W3D32(body_codebook)
