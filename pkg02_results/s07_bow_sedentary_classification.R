...
# Adding 33 sedentary tasks
set.seed(5855)
sedentary.samples <- sample(which(dataset.df$class.label == "YES"), size = 100, replace = T)
...
# Random Forest
RF.out <- myRandomForest(dataset.df[, c(1, 4:(ncol(dataset.df)))], ntree = 20000)
...
# Does PCA help?--------------------------------------------
pca.out <- princomp(dataset.df[, 4:(ncol(dataset.df) - 1)], cor = T)
summary(pca.out)
pca.df <- data.frame(PID = dataset.df$PID, pca.out$scores[, 1:8], class.label = dataset.df$class.label)
RF3.out <- myRandomForest(pca.df)
print.classificationStatistics(RF3.out)
