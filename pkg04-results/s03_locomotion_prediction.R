...
df_bow$class.label <- factor(bow.df$class.locomotion, levels = c("Yes", "No"))
df_stat <- stat.df[, -c(1:2, 10:12)]
df_stat$class.label <- factor(stat.df$class.locomotion, levels = c("Yes", "No"))

# Results on statistical variables
stat.staudenmayer <- Staudenmayer.locomotion.decisionTree(df_stat)
